...
def payments(request):
    ...
    if request.headers.get('x-requested-with') == 'XMLHttpRequest' and request.method == 'POST':
        ...
        # SEND ORDER RECEIVED EMAIL TO THE merchant
        mail_subject = 'You have received a new order.'
        mail_template = 'orders/new_order_received.html'
        to_emails = []
        for i in cart_items:
            if i.product_item.merchant.user.email not in to_emails:
                to_emails.append(i.product_item.merchant.user.email)

                ordered_food_to_merchant = OrderedFood.objects.filter(order=order, product_item__merchant=i.product_item.merchant)
                print(ordered_food_to_merchant)

        
                context = {
                    'order': order,
                    'to_email': i.product_item.merchant.user.email,
                    'ordered_food_to_merchant': ordered_food_to_merchant,
                    'merchant_subtotal': order_total_by_merchant(order, i.product_item.merchant.id)['subtotal'],
                    'tax_data': order_total_by_merchant(order, i.product_item.merchant.id)['tax_dict'],
                    'merchant_grand_total': order_total_by_merchant(order, i.product_item.merchant.id)['grand_total'],
                }
                send_notification(mail_subject, mail_template, context)
